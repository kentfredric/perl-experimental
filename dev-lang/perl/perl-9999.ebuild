...
EAPI=2
EGIT_REPO_URI='git://perl5.git.perl.org/perl.git'
EGIT_BRANCH='blead'
inherit eutils alternatives flag-o-matic toolchain-funcs multilib git

PATCH_VER=5

PERL_OLDVERSEN="5.10.1"
PERL_VER="5.11.0"
...
src_unpack(){
	ewarn "This downloads a full Git clone of the Perl repository, weighing in"
	ewarn "around 100mb";
	ewarn "You're going to want diskspace, and fetch time";
	ewarn "If this is not ok, abort now"
	epause 2;
	git_src_unpack;
}
dual_scripts() {
...
pkg_setup() {
	...
	ewarn "*** WHOA THERE!!! ***"
	ewarn ""
	ewarn "This is an UNSTABLE DEVELOPMENT release."
	ewarn ""
	ewarn "Do ***NOT*** install this into production use."
	ewarn "Data corruption and crashes are possible."
	ewarn ""
	ewarn "It is most seriously suggested that you do not continue any further"
	ewarn "unless you want to help in developing and debugging Perl."
	ewarn ""
	ewarn "If you *still* want to build this perl, set PERL_DEVEL_VERSION_OK=1"
	ewarn ""
#	if has "${PERL_DEVEL_VERSION_OK}" 1 ; then
#		local dopause=1
#	else
#		die;
#	fi
...
src_install() {
	...
	make DESTDIR="${D}" ${installtarget} || die "Unable to make
	${installtarget}"

	rm "${D}"/usr/bin/perl
	#ln -s perl${PERL_VER} "${D}"/usr/bin/perl

	dolib.so "${D}"/${coredir}/${LIBPERL} || die
	einfo dolib.so "${D}"/${coredir}/${LIBPERL}

#	dosym ${LIBPERL} /usr/$(get_libdir)/libperl$(get_libname ${SHORT_PV}) || die
#	einfo dosym ${LIBPERL} /usr/$(get_libdir)/libperl$(get_libname ${SHORT_PV}) 

#	dosym ${LIBPERL} /usr/$(get_libdir)/libperl$(get_libname) || die
#	einfo dosym ${LIBPERL} /usr/$(get_libdir)/libperl$(get_libname) || die

	rm "${D}"/${coredir}/${LIBPERL}
	einfo rm "${D}"/${coredir}/${LIBPERL}

	dosym ../../../../../$(get_libdir)/${LIBPERL} ${coredir}/${LIBPERL}
	einfo dosym ../../../../../$(get_libdir)/${LIBPERL} ${coredir}/${LIBPERL}

	dosym ../../../../../$(get_libdir)/${LIBPERL} ${coredir}/libperl$(get_libname ${SHORT_PV})
	einfo dosym ../../../../../$(get_libdir)/${LIBPERL} ${coredir}/libperl$(get_libname ${SHORT_PV})

	dosym ../../../../../$(get_libdir)/${LIBPERL} ${coredir}/libperl$(get_libname)
	einfo dosym ../../../../../$(get_libdir)/${LIBPERL} ${coredir}/libperl$(get_libname)

	rm -rf "${D}"/usr/share/man/man1 || die "Unable to remove module man pages"
...
#	done
	...
	dosed 's:./miniperl:/usr/bin/perl:' /usr/bin/xsubpp${PERL_VER}
	fperms 0755 /usr/bin/xsubpp${PERL_VER}
